 ...
 */
@Service
public class DocumentLevelAccessService {
    @Autowired
    private DataSourceConnection ds;

    public ResponseModel create(DocumentLevelAccessEntity d) {
        ...
        try {
            CallableStatement cs = cn.prepareCall("{call sp_DocumentLevelAccessCreate(?,?,?)}");
    ...
    public ResponseModel update(DocumentLevelAccessEntity d) {
        int updates = 0;
        Connection cn = this.ds.openConnection();
        try {
            CallableStatement cs = cn.prepareCall("{call sp_DocumentLevelAccessUpdate(?,?,?,?,?)}");
            cs.setInt(1, d.getId());
    ...
    public ResponseModel delete(int id) {
        ...
        Connection cn = this.ds.openConnection();
        try {
            CallableStatement cs = cn.prepareCall("{call sp_DocumentLevelAccessDelete(?)}");
            cs.setInt(1, id);
    ...
    public ResponseModel list() {
        ...
        Connection cn = this.ds.openConnection();
        try {
            CallableStatement cs = cn.prepareCall("{call sp_DocumentLevelAccessList()}");
            // Muestra las filas / columnas 
            ResultSet rs = cs.executeQuery();
            while (rs.next()) {
                // Creación objeto d para obtener las variables de la clase DocumentsGroupEntity
                DocumentLevelAccessEntity d = new DocumentLevelAccessEntity();
    ...
    public ResponseModel findById (int id) {
        DocumentLevelAccessEntity d = null;
        Connection cn = this.ds.openConnection();
        try {
            //cn.setAutoCommit(false);
            CallableStatement cs = cn.prepareCall("{call sp_DocumentLevelAccessFindById(?)}");
            cs.setInt(1, id);
            ResultSet rs = cs.executeQuery();
            while (rs.next()) {
                d = new DocumentLevelAccessEntity();
                d.setId(rs.getInt("id"));
                d.setCode(rs.getString("code"));
                d.setDescription(rs.getString("description"));
                d.setActive(rs.getBoolean("active"));
