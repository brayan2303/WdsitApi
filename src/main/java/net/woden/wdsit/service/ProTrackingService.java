 ...
 */
@Service
public class ProTrackingService {
    
    @Autowired
    private DataSourceConnection ds;

    public ResponseModel create(ProTrackingEntity b) {
        int inserts = 0;
        Connection cn = this.ds.openConnection();

        try {
            cn.setAutoCommit(false);
            CallableStatement cs = cn.prepareCall("{call sp_ProTrackingCreate(?,?,?,?,?)}");
    ...
    public ResponseModel update(ProTrackingEntity b) {
        int updates = 0;
        Connection cn = this.ds.openConnection();

        try {
            cn.setAutoCommit(false);
            CallableStatement cs = cn.prepareCall("{call sp_ProTrackingUpdate(?,?,?,?,?)}");
            cs.setInt(1,b.getId());
    ...
    public ResponseModel approveReject(int trackingId,String status) {
        ...
        try {
            cn.setAutoCommit(false);
            CallableStatement cs = cn.prepareCall("{call sp_ProTrackingApproveReject(?,?,?)}");
    ...
    public ResponseModel delete(int trackingId) {
        ...
        try {
            cn.setAutoCommit(false);
            CallableStatement cs = cn.prepareCall("{call sp_ProTrackingDelete(?)}");
            cs.setInt(1, trackingId);
    ...
    public ResponseModel list(int actionPlanId) {
        ArrayList<ProTrackingEntity>list=new ArrayList();
        Connection cn = this.ds.openConnection();

        try {
            cn.setAutoCommit(false);
            CallableStatement cs = cn.prepareCall("{call sp_ProTrackingList(?)}");
            cs.setInt(1, actionPlanId);
            ResultSet rs=cs.executeQuery();
            while(rs.next()){
                ProTrackingEntity b=new ProTrackingEntity();
